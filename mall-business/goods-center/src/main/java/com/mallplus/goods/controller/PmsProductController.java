...
public class PmsProductController {
    ...
    public Object goodsStatic() throws Exception {
        List<PmsProduct> goodsList = IPmsProductService.list(new QueryWrapper<PmsProduct>());
        int onCount = 0;
        int offCount = 0;
        int nowCount = 0;
        for (PmsProduct goods : goodsList) {
            ...
            if (DateUtil.formatDate(goods.getCreateTime()).equals(DateUtil.formatDate(new Date()))) {
                nowCount++;
            }
        }
